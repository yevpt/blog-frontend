...
vi.mock("./moment-card", () => ({
  MomentCard: ({
    ...
    onComment?: (moment: { id: number; content: string }) => void;
    priority?: boolean;
  }) => (
    <div data-testid="moment-card" data-priority={priority ?? false}>
...
function getMomentColumns(): HTMLElement[] {
  return Array.from(
...
describe("MomentsList", () => {
  ...
  it("无数据时显示空状态", () => {
    render(<MomentsList initialPage={makePageResp({ list: [], total: 0 })} />);
  ...
  it("pages > 1 时显示加载更多哨兵", () => {
    render(<MomentsList initialPage={makePageResp({ total: 40, pages: 2 })} />);
  ...
  it("pages === 1 时显示到底提示", () => {
    render(<MomentsList initialPage={makePageResp({ total: 1, pages: 1 })} />);
  ...
  it("从全部切到博主时重算瀑布流列分配", async () => {
    ...
    await waitFor(() => {
      const [firstColumn, secondColumn] = getMomentColumns();
