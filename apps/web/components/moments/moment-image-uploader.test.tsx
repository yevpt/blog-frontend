...
function Harness({ initial = [] as MomentImageItem[] }) {
  const [items, setItems] = useState<MomentImageItem[]>(initial);
  return <MomentImageUploader items={items} onChange={setItems} />;
}
...
describe("MomentImageUploader", () => {
  ...
  it("已有 9 张时不再渲染「添加」格", () => {
    const nine: MomentImageItem[] = Array.from({ length: 9 }, (_, i) => ({
      id: String(i),
      file: img(),
      previewUrl: "blob:" + i,
    }));
    render(<MomentImageUploader items={nine} onChange={() => {}} />);
  ...
  it("HEIC 转换失败时展示可操作提示", async () => {
    ...
    render(<Harness />);
    await user.upload(
      screen.getByTestId("moment-image-input") as HTMLInputElement,
      img("photo.heic"),
    );
