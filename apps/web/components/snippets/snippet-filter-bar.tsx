...
export type SnippetTab = "all" | "owner" | "friends";
export type SnippetSort = "latest" | "popular";

interface SnippetFilterBarProps {
  activeTab: SnippetTab;
  onTabChange: (tab: SnippetTab) => void;
  activeSort: SnippetSort;
  onSortChange: (sort: SnippetSort) => void;
}

const TAB_ITEMS: Array<{ id: SnippetTab; label: string }> = [
  { id: "all", label: "全部" },
  { id: "owner", label: "博主" },
...
function getNextSort(sort: SnippetSort): SnippetSort {
  return sort === "latest" ? "popular" : "latest";
}

/** 顺序图标 + 文字，点击在「最新 / 最热」间切换 */
function SnippetSortToggle({
  activeSort,
  onSortChange,
}: {
  activeSort: SnippetSort;
      ...
      <div className="border-b border-border">
        <div className="flex items-end justify-between gap-6">
          <Tabs
            ...
            onSelectionChange={(key) => onTabChange(key as SnippetTab)}
          ...
          <div className="shrink-0 pb-1 pr-1">
            <SnippetSortToggle activeSort={activeSort} onSortChange={onSortChange} />
          </div>
        </div>
      </div>
    </div>
