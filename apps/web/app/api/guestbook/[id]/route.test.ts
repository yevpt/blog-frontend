...
describe("/api/guestbook/[id]", () => {
  beforeEach(() => vi.clearAllMocks());

  it("DELETE 委托给 proxyDelete 转发到 /guestbook/{id}", async () => {
    ...
    expect(proxyDelete).toHaveBeenCalledWith(req, "/guestbook/9");
  });

  it("PATCH 委托给 proxyPatch 转发到 /guestbook/{id}（Idempotency-Key 由公共代理透传）", async () => {
    ...
    expect(proxyPatch).toHaveBeenCalledWith(req, "/guestbook/9");
  });
});
